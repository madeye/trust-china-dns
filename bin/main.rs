...
async fn socks5_lookup(qname: &str, qtype: QueryType, socks5: SocketAddr, ns: SocketAddr) -> Result<DnsPacket> {
    ...
    let mut req_buffer = BytePacketBuffer::new();
    packet.write(&mut req_buffer).unwrap();
    let size = req_buffer.pos;
    let mut size_buffer: [u8; 2] = [
        ((size >> 8) & 0xFF) as u8,
        ((size >> 0) & 0xFF) as u8,
    ];
    let mut send_buffer: [u8; 512 + 2] = [0; 512 + 2];
    send_buffer[..2].copy_from_slice(&size_buffer[..2]);
    send_buffer[2..size+2].copy_from_slice(&req_buffer.buf[0..size]);
...
async fn acl_lookup(
    ...
    let ip_bypassed = match local_response.get_random_a() {
        Some(a) => {
            let addr = Address::SocketAddress(SocketAddr::new(IpAddr::V4(a), 0));
            acl.check_target_bypassed(&addr).await
        }
        None => match local_response.get_random_aaaa() {
            Some(aaaa) => {
                let addr = Address::SocketAddress(SocketAddr::new(IpAddr::V6(aaaa), 0));
                acl.check_target_bypassed(&addr).await
            }
            None => false,
        },
...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        match socket.send_to(data, src).await {
            Ok(_) => {}
            Err(e) => {
